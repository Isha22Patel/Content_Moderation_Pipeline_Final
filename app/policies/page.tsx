...
export default function PolicySettings() {
  ...
  useEffect(() => {
    api.policies().then((p) => {
      setPolicies(p);
      const match = p.find((x: any) => x.id === selectedId) || p[0];
  ...
  function updateCat(catId: string, patch: any) {
    if (!draft) return;
  ...
  async function save() {
    ...
    setError(null);
    try {
      const updated = await api.policyUpdate(draft.id, draft);
  ...
  async function reset() {
    try {
      const fresh = await api.policyReset();
      setPolicies(fresh);
      const match = fresh.find((p: any) => p.id === selectedId) || fresh[0];
      if (match) { setSelectedId(match.id); setDraft(JSON.parse(JSON.stringify(match))); }
  ...
  function removeRule(id: string) {
    setDraft({ ...draft, customRules: draft.customRules.filter((r: any) => r.id !== id) });
  }
