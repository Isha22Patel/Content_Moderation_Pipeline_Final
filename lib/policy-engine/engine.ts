...
import { CATEGORY_LABELS } from '../moderation/constants';

const ACTION_RANK: Record<string, number> = { allow: 0, review: 1, block: 2 };

function moreSevere(a: string, b: string) {
...
/** Load a platform's full policy from DB. */
export async function getPolicy(platformId: string) {
...
/**
 * Pure, deterministic policy application.
 * Takes a classification result + policy, returns a routing decision.
 */
export function applyPolicy(content: string, classification: any, policy: any) {
  const catMap: Record<string, any> = {};
  if (policy && policy.categoryPolicies) {
    for (const c of policy.categoryPolicies) {
      catMap[c.category] = c;
    }
  }
  ...
  for (const flag of classification.flags) {
    ...
    if (flag.confidence >= cat.autoActionThreshold) {
      action = 'block';
      explanation = `${label} confidence ${pct(flag.confidence)} ≥ auto-block threshold ${pct(cat.autoActionThreshold)} → automatically blocked.`;
    } else if (flag.confidence >= cat.reviewThreshold) {
      action = 'review';
      explanation = `${label} confidence ${pct(flag.confidence)} is in review band [${pct(cat.reviewThreshold)}, ${pct(cat.autoActionThreshold)}) → sent to human review.`;
    } else {
      action = 'allow';
      explanation = `${label} confidence ${pct(flag.confidence)} < review threshold ${pct(cat.reviewThreshold)} → below the bar, allowed.`;
    }
