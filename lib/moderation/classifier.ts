...
import { HARM_CATEGORIES, CATEGORY_LABELS } from './constants';
...
function buildPrompt(content: string, context: Context) {
...
export async function classify(content: string, context: Context) {
  ...
  let rawFlags = parsed.flags || parsed.categories || parsed.results || parsed.classifications || parsed.analysis || [];

  if (!Array.isArray(rawFlags) && typeof rawFlags === 'object') {
    rawFlags = Object.entries(rawFlags).map(([category, data]) => ({
      category,
      ...(typeof data === 'object' && data !== null ? data : { confidence: Number(data) || 0 }),
    }));
  }

  const flags = rawFlags
    .filter((f: any) => (Number(f.confidence) || 0) >= 0.1)
    .map((f: any) => ({
      category: f.category,
